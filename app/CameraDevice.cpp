...
void CameraDevice::get_live_view_and_OSD()
{
    ...
    unsigned char* composit_image_buff = nullptr;
    ...
    do {
        ...
        {
            // FAILED
            tout << "GetLiveView FAILED (new OSD Image buffer)\n";
            err = SDK::CrError_Generic;
            break;
        }
        ...
        if (CR_FAILED(err)) {
            tout << "GetLiveView FAILED (GetLiveViewProperties)\n";
            break;
        }
        SDK::ReleaseLiveViewProperties(m_device_handle, property);
        ...
        err = SDK::GetLiveViewImageInfo(m_device_handle, &inf);
        if (CR_FAILED(err)) {
            tout << "GetLiveView FAILED (GetLiveViewImageInfo)\n";
            break;
        }
        ...
        if (bufSize < 1)
        {
            tout << "GetLiveView FAILED (Buffer size=0)\n";
            err = SDK::CrError_Generic;
            break;
        }
        ...
        if (CR_FAILED(err))
        {
            ...
            if (err == SDK::CrWarning_Frame_NotUpdated) {
                ...
                tout << "Error GetLiveView FAILED GetLiveViewImage\n";
            }
            break;
        }
    ...
    if (validOSD)
    {
        ...
        do
        {
            ...
            composit_image_buff = new unsigned char[CR_OSD_IMAGE_MAX_SIZE];
            if (!composit_image_buff) {
                // FAILED
                tout << "GetLiveView FAILED (Composite buffer)\n";
                err = SDK::CrError_Generic;
                break;
            }

            // Composite LiveviewImage and OSDImage
            CrInt32u composit_image_size = 0;
            bool cverr = OpenCVWrapper::CompositeImage(lvbuf, osd_image_data, composit_image_buff, &composit_image_size);

            if (!cverr || (0 == composit_image_size)) {
                // FAILED
            ...
            std::ofstream file(path, std::ios::out | std::ios::binary);
            if (file.bad()) {
                tout << "Failed to create file.\n";
                err = SDK::CrError_Generic;
            }
            else {
                file.write((char*)composit_image_buff, composit_image_size);
                file.close();
            }
...
void CameraDevice::change_live_view_enable()
{
    ...
    m_lvEnbSet = (current == SDK::CrDeviceSetting_Disable) ? true : false; // reverse
